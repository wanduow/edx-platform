...
@ensure_csrf_cookie
@cache_control(no_cache=True, no_store=True, must_revalidate=True)
@require_query_params('unique_student_identifier')
@require_level('staff')
def mark_student_can_skip_entrance_exam(request, course_id):  # pylint: disable=invalid-name
    ...
    if created:
        message = _('Student %s is set to skip entrance exam.') % student_identifier
    else:
        message = _('Student %s is already set to skip entrance exam.') % student_identifier
    response_payload = {
        'message': message,
    }
    return JsonResponse(response_payload)
